...
with tab_reg:
    st.header("📈 Regression: Manual Input on 5 Key Variables")
    if not numeric_cols:
        ...
        st.subheader("Manual Input for Prediction (only 5 key variables)")
        # Only use these for input, rest set to mean/mode
        ...
        if submitted:
            # Build one sample row with only those 5 set to user value; others set to mean/mode
            back_dict = {}
            for col in X_orig.columns:
                if col in key_inputs:
                    back_dict[col] = input_dict[col]
                elif str(X_orig[col].dtype).startswith("float") or str(X_orig[col].dtype).startswith("int"):
                    back_dict[col] = float(df[col].mean())
                else:
                    back_dict[col] = df[col].mode().iloc[0]
            inp_df = pd.DataFrame([back_dict])
